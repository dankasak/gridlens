...
| `grid-lens-advisory-card` | Plan status header (plan name/solver/last-run time, status badge), control-mode timeline, deferrable-load recommendations. `compact: true` config renders just the header — used as a slim "optimiser & plan" status bar at the top of the Power Flow view; `title` config overrides the header text in that mode. `show_current_rates: true` adds a one-line buy/sell readout ("Buy 22c/kWh · Sell 3c/kWh") under the plan-status line — the rate for the slot covering now, from the same `trajectory` attribute. Just the numbers; the rate *graph* is `grid-lens-price-chart-card`. The seed enables it on the compact bar. Works in the full card too. |
...
optimiser last run" is visible without switching to the Battery Plan view. The status bar
carries `show_current_rates: true` (2026-09-10), so it also shows the current buy/sell
rate as a one-line readout under the plan name.
